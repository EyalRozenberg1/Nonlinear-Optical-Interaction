function [A, P] = WavePropagation_SSF_2017_03_20(Undepleted, Lambda, w0, NumOfPoints, PlaneGauss_, dx_prop, CrystalPropAxis, DeltaK, K, Omega, n, I, InteractionType, deff, A_from_I, Kappa, P_from_A, samples)
    ...
    switch InteractionType
        case 'Type1 SHG'
            if(PlaneGauss_) % Plane Wave
                ...
                %zr.out      = zr.in1;
                zr.out      = pi*n.out(1)*w0.out^2/Lambda.out;     % [m] Reighley range with consider of Lambda out
                ...
                rmax.max        = max(rmax.in1, rmax.out); %0.7*max(rmax.in1, rmax.out); 1.2*max(rmax.in1, rmax.out);
